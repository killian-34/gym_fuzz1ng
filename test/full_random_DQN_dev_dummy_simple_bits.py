...
def main(n_episodes=2000, max_t=10, eps_start=1.0, eps_end=0.01, eps_decay=0.995, trial=0):
    ...
    import pandas as pd 
    pd.DataFrame(times,columns=['tt4','tt5','tt6','tt7','tt8s']).to_csv('random_times_%s_%s.csv'%(trial,INPUT_SIZE),index=False)

    pd.DataFrame(edits_until_x,columns=['et4','et5','et6','et7','et8s']).to_csv('random_edits_until_x_%s_%s.csv'%(trial,INPUT_SIZE),index=False)

    pd.DataFrame([unique_transitions_found]).to_csv('random_transitions_per_edit_%s_%s.csv'%(trial,INPUT_SIZE),index=False)
